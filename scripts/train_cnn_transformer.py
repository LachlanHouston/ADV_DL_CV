...
save_dir = 'results_hybrid_transformer_unet_v3' # New directory for corrected version
model_save_path = os.path.join(save_dir, 'hybrid_layer_generator_v3.pth')
...
def save_image_grid(input_images, target_images, predicted_images, epoch, save_dir='results'):
    ...
    for i in range(n_images):
        ...
        axes[0, i].axis('off'); axes[0, i].set_title('Input', pad=10) if i == 0 else None
        axes[1, i].imshow(target_images[i].squeeze().numpy(), cmap=cmap)
        axes[1, i].axis('off'); axes[1, i].set_title('Target', pad=10) if i == 0 else None
        axes[2, i].imshow(predicted_images[i].squeeze().numpy(), cmap=cmap)
        axes[2, i].axis('off'); axes[2, i].set_title('Predicted', pad=10) if i == 0 else None
